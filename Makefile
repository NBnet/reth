...
.PHONY: install_with_expander
install_with_expander: install_dep ## Build and install the reth binary under `~/.cargo/bin`.
	cargo +nightly install --path bin/reth --bin reth --force --locked \
		--features "expander $(FEATURES)" \
		--profile "$(PROFILE)" \
		$(CARGO_INSTALL_EXTRA_FLAGS)

.PHONY: build_with_expander
build_with_expander: install_dep ## Build the reth binary into `target` directory.
	cargo +nightly build --bin reth --features "expander $(FEATURES)" --profile "$(PROFILE)"

.PHONY: install
install: install_dep ## Build and install the reth binary under `~/.cargo/bin`.
...
.PHONY: install-op
install-op: install_dep ## Build and install the op-reth binary under `~/.cargo/bin`.
...
.PHONY: build
build: install_dep ## Build the reth binary into `target` directory.
	cargo build --bin reth --features "$(FEATURES)" --profile "$(PROFILE)"

.PHONY: build-debug
build-debug: install_dep ## Build the reth binary into `target/debug` directory.
	cargo build --bin reth --features "$(FEATURES)"

.PHONY: build-op
build-op: install_dep ## Build the op-reth binary into `target` directory.
	cargo build --bin op-reth --features "optimism $(FEATURES)" --profile "$(PROFILE)" --manifest-path crates/optimism/bin/Cargo.toml

# Builds the reth binary natively.
build-native-%: install_dep
	cargo build --bin reth --target $* --features "$(FEATURES)" --profile "$(PROFILE)"

op-build-native-%: install_dep
